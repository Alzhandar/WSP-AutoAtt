...
def attend_bot(username: str, password: str):
    ...
    session_dir = os.path.join(os.getcwd(), SETTINGS.chrome_sessions_dir, session_id)
...
def analyze_current_page(driver):
    ...
    try:
        ...
        key_checks = {
            "Страница входа": is_login_page(driver),
...
def check_other_pages(selenium_driver):
    ...
    for url, name in pages_to_check:
        try:
            ...
            print(f"Проверяем {name}: {target_url}")
            ...
            if is_login_page(selenium_driver):
                print(f"Требуется повторная авторизация")
                return False
...
def try_to_attend(selenium_driver):
    ...
    try:
        ...
        if button_divs:
            ...
            print("ℹ  Кнопки 'Отметиться' не найдены")
...
def login(selenium_driver):
    ...
    if submit_button is not None:
        ...
        for attempt in range(max_attempts):
            ...
            if not is_login_page(selenium_driver):
    ...
    try:
        ...
        for page_url in pages_to_check:
            try:
                ...
                driver.get(normalize_wsp_url(page_url))
                time.sleep(3)
